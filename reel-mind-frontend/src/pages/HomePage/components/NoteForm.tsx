...
const SUPPORTED_PLATFORM_HOSTS: Record<string, string[]> = {
  douyin: ['douyin.com'],
...
const isPlatformUrl = (value: string, platform: string) => {
  const hosts = SUPPORTED_PLATFORM_HOSTS[platform] || []
...
const formSchema = z
  ...
  .superRefine(({ video_url, platform }, ctx) => {
    ...
    try {
      if (!isPlatformUrl(video_url, platform))
        ctx.addIssue({ code: 'custom', message: '请输入所选平台的视频链接', path: ['video_url'] })
    }
    catch {
...
const NoteForm = () => {
  ...
  useEffect(() => {
    ...
    form.reset({
      platform: formData.platform || 'douyin',
    ...
    <div className="h-full w-full">
      <Form {...form}>
        <form onSubmit={form.handleSubmit(onSubmit, onInvalid)} className="space-y-4">
          ...
          {/* 视频链接 & 平台 */}
          <SectionHeader title="视频链接" tip="支持抖音精选、B站、快手视频" />
          <div className="flex gap-2">
            {/* 平台选择 */}
            ...
            <FormField
              ...
              render={({ field }) => (
                <FormItem className="flex-1">
                  <Input disabled={!!editing} placeholder="请输入抖音精选、B站或快手视频链接" {...field} />
                  <FormMessage style={{ display: 'none' }} />
                </FormItem>
              )}
            />
