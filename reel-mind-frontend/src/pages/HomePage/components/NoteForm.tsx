...
import { ChevronDown, Info, Loader2, Plus, SearchCheck } from 'lucide-react'
import { Alert, AlertDescription } from '@/components/ui/alert.tsx'
import { create_verification_task, rerun_verification_task, type TaskSnapshot } from '@/services/note.ts'
...
const NoteForm = () => {
  ...
  useEffect(() => {
    ...
    form.reset({
      ...
      collection_folder: formData.collection_folder || currentTask.collection?.folder || '核验历史',
    ...
    <div className="h-full w-full">
      <Form {...form}>
        <form onSubmit={form.handleSubmit(onSubmit, onInvalid)} className="space-y-4">
            ...
            <div className={editing ? 'grid grid-cols-[minmax(0,1fr)_112px] gap-2' : ''}>
              ...
              >
                ...
                <span className="truncate">
                  {generating ? '正在核实…' : editing ? '重新联网核实' : '开始联网核实'}
                </span>
              </button>
              {editing && (
                <Button
          ...
          {/* 核实内容 */}
          <div>
            <SectionHeader title="核实内容" tip="粘贴网页 URL 或直接输入一段待核实文本" />
            <div>
              <FormField
                ...
                render={({ field }) => (
                  <FormItem>
                    <Textarea
                      disabled={!!editing}
                      placeholder="输入网页 URL，或粘贴要联网核实的文本。长文本会被拆成 atomic claims 后逐条联网核验。"
          ...
          {/* 核验模型 */}
          <div>
            <SectionHeader title="核验模型" tip="不同模型效果不同，建议自行测试" />
            {modelList.length > 0 ? (
              <FormField
                control={form.control}
                name="model_name"
