...
const fallbackStatus: DeployStatus = {
    backend: {
        status: 'unknown',
        port: 0,
    },
...
export default function Monitor() {
    ...
    const fetchStatus = useCallback(async () => {
        try {
            ...
            setStatus({
                backend: data?.backend ?? fallbackStatus.backend,
                cuda: data?.cuda ?? fallbackStatus.cuda,
                whisper: data?.whisper ?? fallbackStatus.whisper,
                ffmpeg: data?.ffmpeg ?? fallbackStatus.ffmpeg,
            })
            setLastUpdated(new Date())
        } catch (e) {
            const message = e && typeof e === 'object' && 'msg' in e ? String((e as { msg?: string }).msg) : '无法连接到后端服务'
            setError(message)
            setStatus(null)
        } finally {
            setLoading(false)
