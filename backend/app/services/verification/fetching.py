...
from app.services.academic_evidence import OFFICIAL_ACADEMIC_HOSTS
...
def _pdf_snapshot(url: str, content: bytes) -> dict:
        ...
        try:
            ...
            try:
                ...
                with pdfplumber.open(BytesIO(content)) as pdf:
                    ...
                    page_count_parsed = min(page_count_total, PDF_PAGE_LIMIT)
                    text, page_spans = _pdf_text_with_spans(
                        [(page.extract_text() or "") for page in pdf.pages[:PDF_PAGE_LIMIT]]
                    )
                parser = "pdfplumber"
    ...
    return {
        ...
        "authors": [str(pdf_metadata.get("author"))] if pdf_metadata.get("author") else [],
        "published_at": str(pdf_metadata.get("creationDate") or pdf_metadata.get("creationdate") or ""),
        "doi": "",
        "venue": "",
        "pdf_url": url,
        "retrieved_at": datetime.now(timezone.utc).isoformat(),
