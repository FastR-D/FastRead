...
当前扩展范围是 verification-first popup：从当前页面 URL 或粘贴文本创建 FastRead 联网核实任务。抖音 Cookie 同步只保留为 Douyin 输入诊断，不再是插件主功能。
...
桌面 web 端（`fastread-frontend/`）继续负责供应商/模型管理、核实历史、报告查看和重跑。插件当前负责从浏览器上下文快速发起联网核实任务。
