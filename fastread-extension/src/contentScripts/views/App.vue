...
const label = computed(() => platform ? '用 FastRead 提取这个视频' : '')
...
async function trigger() {
  ...
  toast.value = null
  try {
    const res = await sendMessage('fastread-start', {
...
<template>
  <div v-if="platform" class="fastread-fab fixed bottom-24 right-6 z-[2147483647] flex flex-col items-end gap-2 font-sans select-none">
    <div
      v-if="toast"
