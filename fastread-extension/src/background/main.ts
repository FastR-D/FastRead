...
onMessage<{ url: string }, 'fastread-start'>('fastread-start', async ({ data, sender }) => {
...
browser.runtime.onInstalled.addListener(() => {
  console.log('FastRead extension installed')

  // 右键菜单：在视频页或视频链接上发起知识提取
  try {
    browser.contextMenus.create({
      id: 'fastread-summarize-page',
      title: '用 FastRead 提取视频',
...
browser.contextMenus?.onClicked.addListener(async (info, tab) => {
  if (info.menuItemId !== 'fastread-summarize-page')
    return
