...
1. 确认已经准备好本地依赖：`backend\.venv` 和 `reel-mind-frontend\node_modules`。
2. 双击 `run.bat`。
3. 等待后端和前端窗口启动完成。
4. 浏览器自动打开后，访问地址是：
...
- 前端：进入 `reel-mind-frontend` 后执行 `corepack enable` 和 `pnpm install`
...
如果你使用旧抖音视频笔记兼容流程，再检查抖音 Cookie 是否已通过浏览器扩展同步。
