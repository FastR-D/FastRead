...
欢迎为 Reel Mind 贡献代码。本文档约定分支管理、提交规范、合并流程。新贡献者请通读一遍后再开 PR。
...
| `reel-mind-frontend/` | React 19 + Vite | `pnpm install && pnpm dev` |
| `reel-mind-extension/` | Vue 3 + Vite + WebExtension MV3 | `pnpm install && pnpm dev` |
...
- `extension` — 浏览器插件（`reel-mind-extension/`）
- `frontend` — Web 前端（`reel-mind-frontend/`）
...
git tag -a v<版本号> -m "Reel Mind v<版本号>" && git push origin v<版本号>
...
`reel-mind-extension/` 早期使用 `simple-git-hooks` 的 `postinstall`，会在仓库根目录 `.git/hooks/pre-commit` 注入 `pnpm lint-staged`，但仓库根没有 `package.json`，导致**任何 commit 都被钩子卡死**。**已在 v2.1.0 起移除**该 postinstall 配置。
