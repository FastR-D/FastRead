...
import { DEFAULT_SETTINGS, LEGACY_SETTINGS_KEY, MAX_TASKS, SETTINGS_KEY, TASKS_KEY } from '~/logic/constants'
...
async function readSettings(): Promise<Settings> {
  const obj = await browser.storage.local.get([SETTINGS_KEY, LEGACY_SETTINGS_KEY])
  const raw = (obj[SETTINGS_KEY] || obj[LEGACY_SETTINGS_KEY]) as string | undefined
  if (!obj[SETTINGS_KEY] && obj[LEGACY_SETTINGS_KEY])
    await browser.storage.local.set({ [SETTINGS_KEY]: obj[LEGACY_SETTINGS_KEY] })
  if (!raw)
...
async function startTask(url: string): Promise<{ ok: boolean, taskId?: string, error?: string }> {
  ...
  if (!platform)
    return { ok: false, error: '当前链接不是抖音精选、B站或快手视频链接' }
...
onMessage<{ url: string }, 'reel-mind-start'>('reel-mind-start', async ({ data, sender }) => {
...
browser.runtime.onInstalled.addListener(() => {
  console.log('Reel Mind extension installed')

  // 右键菜单：在视频页或视频链接上发起知识提取
  try {
    browser.contextMenus.create({
      id: 'reel-mind-summarize-page',
      title: '用 Reel Mind 提取视频',
...
browser.contextMenus?.onClicked.addListener(async (info, tab) => {
  if (info.menuItemId !== 'reel-mind-summarize-page')
    return
