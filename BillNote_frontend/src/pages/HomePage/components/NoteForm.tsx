...
const DOUYIN_URL_PATTERN = /https?:\/\/[^/\s]*douyin\.com\/\S+/i

/* -------------------- 校验 Schema -------------------- */
const formSchema = z
  .object({
    video_url: z.string().optional(),
    platform: z.string().refine(value => value === 'douyin', '当前仅支持抖音精选'),
  ...
  .superRefine(({ video_url, platform }, ctx) => {
    if (platform !== 'douyin') {
      ctx.addIssue({ code: 'custom', message: '当前仅支持抖音精选', path: ['platform'] })
      return
    }
    ...
    try {
      const matchedUrl = video_url.match(DOUYIN_URL_PATTERN)?.[0] ?? video_url
      const url = new URL(matchedUrl)
      if (!['http:', 'https:'].includes(url.protocol))
        throw new Error()
      if (!url.hostname.endsWith('douyin.com'))
        ctx.addIssue({ code: 'custom', message: '请输入抖音精选视频链接', path: ['video_url'] })
    }
    catch {
      ctx.addIssue({ code: 'custom', message: '请输入正确的抖音精选视频链接', path: ['video_url'] })
    }
  })

export type NoteFormValues = z.infer<typeof formSchema>

const createEmptyFormValues = (modelName = ''): NoteFormValues => ({
  platform: 'douyin',
  quality: 'medium',
...
const NoteForm = () => {
  ...
  useEffect(() => {
    ...
    form.reset({
      ...
      collection_folder: formData.collection_folder || currentTask.collection?.folder || '默认收藏夹',
      collection_tags: formData.collection_tags || currentTask.collection?.tags?.join('，') || '',
      collection_note: formData.collection_note || currentTask.collection?.note || '',
    ...
    <div className="h-full w-full">
      <Form {...form}>
        <form onSubmit={form.handleSubmit(onSubmit, onInvalid)} className="space-y-4">
          ...
          {/* 视频链接 & 平台 */}
          <SectionHeader title="视频链接" tip="当前仅支持抖音精选知识视频" />
          <div className="flex gap-2">
            {/* 平台选择 */}
            ...
            <FormField
              ...
              render={({ field }) => (
                <FormItem className="flex-1">
                  <Input disabled={!!editing} placeholder="请输入抖音精选视频链接" {...field} />
                  <FormMessage style={{ display: 'none' }} />
                </FormItem>
              )}
            />
          ...
          {/* 视频理解 */}
          <SectionHeader title="收藏信息" tip="用于在我的收藏里分类和检索" />
          <div className="space-y-2">
            <FormField
              ...
              render={({ field }) => (
                <FormItem>
                  <FormLabel>收藏夹</FormLabel>
                  <Input placeholder="默认收藏夹" {...field} />
                  <FormMessage />
                </FormItem>
              )}
            />
